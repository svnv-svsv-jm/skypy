...
for %%l in (%InputPath%*%InputExt%) do (
    ...
    :: Check if a .fbs is present in the input folder with the same name
    if exist %InputPath%%%~nl.fbs (
        ...
        echo %Red%Couldn't find '%InputPath%%%~nl.fbs'. This file is required to convert '%InputPath%%%l'.%White%
    )
)
...
echo Converting '%1'...
:: Run flatc converter
flatc.exe -o %OutputPath% %OutputType% --raw-binary --defaults-json --strict-json "%InputPath%%~n1.fbs" -- "%InputPath%%1"
GOTO :eof
